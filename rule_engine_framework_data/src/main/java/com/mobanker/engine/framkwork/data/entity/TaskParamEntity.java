...
@DatabaseTable(tableName = "RULE_ENGINE:task_param")
public class TaskParamEntity {
    ...
    /** 单个生成rowkey rowkey:taskId(反转补齐10位) - ProductType */
    public void setRowKey() {
        if(StringUtils.isBlank(this.getTaskId().toString())){
            throw new EngineException("taskId为空！");
        }
        this.id = IdUtil.getRowkeyId(this.getTaskId().toString(), this.getProductType());
    }
