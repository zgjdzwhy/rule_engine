...
* @version 1.0 
*/
public class ConditionLengthRange extends EngineConditionExt {

	/** 
	* @Fields serialVersionUID :序列化
	*/
	private static final long serialVersionUID = 1L;
	private static final String BUSI_DESC = "长度范围";	
	...
	public boolean judge(EngineTransferData etd) {
		...
		if(paramObj instanceof String){
			String paramValueStr=(String) paramObj;
			paramValue =new BigDecimal(paramValueStr.length()+"");
		}else if(paramObj instanceof JSONArray){
			JSONArray paramValueList=(JSONArray) paramObj;
			paramValue =new BigDecimal(paramValueList.size()+"");
		}else{
			//如果是数字类型也报错，暂不支持数字长度
			throw new EngineConditionException(EngineUtil.logPrefix(etd, BUSI_DESC)+"参数类型错误,字段名:"+getField()+",参数:"+paramObj.toString());
		}
	...
	public boolean equals(Object obj) {
		if(!(obj instanceof ConditionLengthRange)) return false;
		ConditionLengthRange another = (ConditionLengthRange) obj;	
