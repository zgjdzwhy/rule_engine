...
<body>  
	...
	<form id="uf" method="post" enctype="multipart/form-data">  
		username：<input type="text" name="username"/><br/>productType：<input type="text" name="productType"/><br/>  
		选择要上传的文件：<br/>  
		<input type="file" name="ruleModel" /><span></span><br/>  
	...
	<script>  
        ...
        function upload() {  
            ...
            $.ajax({  
                ...
                url: "http://localhost:8080/ruleModelDesign/saveRuleModel",  
                type: "POST",  
                data: formData,  
				//dataType:'JSONP',
